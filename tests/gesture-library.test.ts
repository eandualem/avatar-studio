...
it("shapes tempo and cycles from energy and sustain and stays valid", () => {
  ...
  expect(shapeGesture(clap, 1.5, 2).repeat).toBeGreaterThan(shapeGesture(clap, 1.5, 0).repeat!);
  for (const gesture of [...seedGestures, ...microGestures])
    for (const energy of [0, 1.5, 3])
      for (const sustain of [0, 2]) motionSchema.parse(shapeGesture(gesture, energy, sustain));
});

it("knows which plans end at rest and gives learned gestures a way home", () => {
  const byName = (name: string) => seedGestures.find((g) => g.name === name)!.motion;
  ...
  const raised = { waypoints: [{ time: 0.6, left: { position: [0.24, 0.98, 0.04] as [number, number, number] } }] };
  expect(normalizeLearned("Wave both hands", raised).finish).toHaveLength(1);
  expect(normalizeLearned("Supported forward kicking pose", raised).finish).toBeUndefined();
...
it("names learned gestures safely and keeps them out of the seed list", () => {
  ...
  Object.assign(globalThis.localStorage, { removeItem: (k: string) => store.delete(k) });
