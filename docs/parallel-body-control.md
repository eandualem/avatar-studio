...
Elias resumed implementation on September 14, 2026 in a fresh session. The
original documentation-only pause is superseded by that explicit request.
Implementation and verification status: [activation notes](parallel-body-verification.md).
This architecture replaces issue #30 and draft PR #31; PR #31 is not merged.
...
- Run independently of Live delegation, with a fresh backend session for each decision.
...
- Voice creation uses `mode: "conversation"`; the isolated runtime starts with
  `VOICE__DELEGATION_ENABLED=false`. Provider delegation has no disabled enum,
  so the runtime also refuses dispatch/workers and restricts data-channel
  commands. The app ignores all backend/delegation events.
...
- `output_mode: "host_tools"` selects native tool-only output. Only final
  `decision: "pending"` admits a tool. `hold` makes no change. Returning a
  receipt on that decision session yields `completed` without a model call.
...
## Historical evidence and checkout preservation

The following describes the pre-implementation handoff. Current deployment and
verification are recorded in [the activation notes](parallel-body-verification.md).
