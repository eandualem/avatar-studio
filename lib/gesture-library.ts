...
export const seedGestures: Gesture[] = [
  ...
  },
  {
    ...
    examples: ["relax", "stand normally", "that's enough, thanks"],
    motion: { waypoints: [{ time: 0.7, ...rest }] },
  },
];
  ...
  return Object.fromEntries([
    ...
    [
      NOT_IN_LIBRARY,
      {
        ...
        examples: ["do a cartwheel", "touch your toes", "kick"],
      },
    ],
  ]);
}
...
/** Tempo and cycles from Jev's energy score (0 still … 3 big and playful). */
export function shapeGesture(gesture: Gesture, energy: number): Motion {
  ...
  if (gesture.cyclic && motion.repeat)
    motion.repeat = clamp(Math.round(motion.repeat * (0.7 + 0.2 * e)), 1, 20);
